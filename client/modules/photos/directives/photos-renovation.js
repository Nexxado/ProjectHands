angular.module('ProjectHands.photos')
    ...
    .directive('photosRenovation', function () {
        return {
            restrict: 'E',
            scope: {album: '@'},
            replace: true,
            templateUrl: 'modules/photos/templates/directives/photos-renovation.html',
            controller: function ($scope, Upload, $timeout, PhotosService, $mdDialog, $mdMedia) {
                ...
                $scope.deletePhoto = function (fileId, index) {
                    ...
                    PhotosService.renoDelete(fileId)
                        .then(function (data) {
                ...
                $scope.upload = function (files) {
                    if (files && files.length) {
                        for (var i = 0; i < files.length; i++) {
                            ...
                            if (!file.$error) {
                                $scope.progress = true;
                                PhotosService.renoUpload($scope.album, file)
                                    .then(function (data) {
